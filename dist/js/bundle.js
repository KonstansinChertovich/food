...
function calc() {
  var calculating = document.querySelector('.calculating__field');
  var genderElements = calculating.querySelectorAll('#gender .calculating__choose-item');
  var activeElements = calculating.querySelectorAll('.calculating__choose_big .calculating__choose-item');
  var input = calculating.querySelectorAll('.calculating__choose_medium input');
  var result = calculating.querySelector('.calculating__result span');
  var female = true,
      factorActive = 1.375,
      height,
      weight,
  ...
  function gettingSex() {
    genderElements.forEach(function (item) {
      item.addEventListener('click', function (e) {
        genderElements.forEach(function (elem) {
  ...
  function getLocalStorage(element, key, classs) {
    element.forEach(function (item) {
      if (item.classList.contains("".concat(classs))) {
        localStorage.setItem("".concat(key), item.getAttribute('id'));
      }
    });
  }
  ...
  function bindingLocalStorage(element, key) {
    if (localStorage.getItem(key)) {
      element.forEach(function (item) {
        ...
        if (item.getAttribute('id') == localStorage.getItem("".concat(key))) {
...
/* harmony import */ var _services_gets__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ../services/gets */ "./src/js/services/gets.js");
...
function menuCard() {
  ...
  var MenuCard = /*#__PURE__*/function () {
    function MenuCard(img, alt, subtitle, descr, coins) {
      _classCallCheck(this, MenuCard);

      this.img = img, this.alt = alt, this.subtitle = subtitle, this.descr = descr, this.coins = coins;
    }
    ...
    _createClass(MenuCard, [{
      ...
      value: function render() {
        ...
        div.innerHTML = "\n                <img src=".concat(this.img, " alt=").concat(this.alt, ">\n                <h3 class=\"menu__item-subtitle\">\u041C\u0435\u043D\u044E ").concat(this.subtitle, "</h3>\n                <div class=\"menu__item-descr\">\u041C\u0435\u043D\u044E ").concat(this.descr, "</div>\n                <div class=\"menu__item-divider\"></div>\n                <div class=\"menu__item-price\">\n                    <div class=\"menu__item-cost\">\u0426\u0435\u043D\u0430:</div>\n                    <div class=\"menu__item-total\"><span>").concat(this.coins, "</span> \u0433\u0440\u043D/\u0434\u0435\u043D\u044C</div>\n                </div>\n            ");
  ...
  (0,_services_gets__WEBPACK_IMPORTED_MODULE_4__["default"])('http://localhost:3000/menu').then(function (data) {
    return data.json();
  }).then(function (data) {
...
/* harmony export */   "showModla": function() { return /* binding */ showModla; },
...
function modal(trigerSelector, modalSelector) {
  ...
  var timeOpenModal = setTimeout(function () {
...
/* harmony import */ var _services_posts__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ../services/posts */ "./src/js/services/posts.js");
...
function post() {
  ...
  function bindDataForm(form) {
    form.addEventListener('submit', function (e) {
      ...
      var json = {};
      formDat.forEach(function (val, key) {
        json[key] = val;
      });
      (0,_services_posts__WEBPACK_IMPORTED_MODULE_5__["default"])('http://localhost:3000/requests', JSON.stringify(json)).then(function (data) {
        console.log(data);
      }).catch(function () {
        console.log('Ошибка');
      }).finally(function () {
...
function slider(_ref) {
  var sliderSelector = _ref.sliderSelector,
      ...
      arrowNextSelector = _ref.arrowNextSelector,
      arrowPrevSelector = _ref.arrowPrevSelector,
      countItem = _ref.countItem,
      scrollItem = _ref.scrollItem;
  ...
  var count = 1; //счетчик слайдов 

  var positionTrack = 0;
  var countItems = countItem; //сколько показывать слайдов в окне

  var scrollItems = scrollItem; //сколько прокручивать за раз

  var slidWidth = slider.clientWidth;
  var colItem = item.length;
  var widthItme = Math.floor(slidWidth / countItems);
  var generalWidthItem = widthItme * colItem;
  item.forEach(function (item) {
    item.style.width = "".concat(widthItme, "px");
    item.style.height = "".concat(slider.clientHeight, "px");
  });
  ...
  function workPrev() {
    if (Math.abs(positionTrack) / widthItme == 1) {
      slidTrack.style.left = "".concat(positionTrack += widthItme, "px");
      checkArrowPrev();
      checkArrowNext();
      changeContent('#current', --count);
    } else {
      slidTrack.style.left = "".concat(positionTrack += scrollItems * widthItme, "px");
      checkArrowPrev();
      checkArrowNext();
      changeContent('#current', --count);
    }
  ...
  function workNext() {
    if ((generalWidthItem - widthItme * countItems - Math.abs(positionTrack)) / widthItme == 1) {
      ...
      slidTrack.style.left = "".concat(positionTrack -= widthItme * scrollItems, "px");
      checkArrowPrev();
      checkArrowNext();
      changeContent('#current', ++count);
    }
  ...
  function changeContent(selector, content) {
    document.querySelector(selector).textContent = "".concat(assayNumber(content));
  }
}
...
function tabs(_ref) {
  var itemTabsSelector = _ref.itemTabsSelector,
      itemSelector = _ref.itemSelector,
      tabContentImgSelector = _ref.tabContentImgSelector,
      activeClass = _ref.activeClass;
  var itemTabs = document.querySelector(itemTabsSelector),
  ...
  function hideItemActive() {
    item.forEach(function (item) {
      return item.classList.remove(activeClass.slice(1));
    });
  }
...
function timer(deadline) {
  ...
  function calcDate(data) {
    var t = Date.parse(data) - Date.parse(new Date()),
  ...
  function setClock() {
    var t = calcDate(deadline).total;
    var timer = setTimeout(function () {
      setClock();
    }, 1000);

    if (t > 0) {
      var days = document.querySelector('.timer__block #days'),
      ...
      clearTimeout(timer);
      document.querySelectorAll('.timer__block span').forEach(function (item) {
